...
describe('Sending the file as part of multipart formdata', () => {
  ...
  test.each(expectedPdfFieldValues)('should contain the value for code %p', ({ code, description, value }) => {
...
describe('Sending the file as a BASE64 encoded string', () => {
  ...
  test.each(expectedPdfFieldValues)('should contain the value for code %p', async ({ code, description, value }) => {
    const entry = response.body?.data?.find((e) => e.code === code);
